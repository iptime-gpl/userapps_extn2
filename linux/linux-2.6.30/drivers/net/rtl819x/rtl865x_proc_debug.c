...
extern int rtk_trunk_port_get(int trk_gid, rtk_portmask_t *trunk_member_portmask);
extern int rtk_trunk_port_set(int trk_gid, rtk_portmask_t trunk_member_portmask);
extern int rtk_trunk_distributionAlgorithm_set(int trk_gid, rtk_trunk_hashVal2Port_t algo_bitmask);

static int32 trunk_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
	rtk_portmask_t	trunk_map;
	int len;

	len = 0;
	rtk_trunk_port_get(0, &trunk_map);
	len += sprintf(page+len," Trunk#1 Map: 0x%x\n", trunk_map.bits[0]);
	rtk_trunk_port_get(1, &trunk_map);
	len += sprintf(page+len," Trunk#2 Map: 0x%x\n", trunk_map.bits[0]);
...
static int32 trunk_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	char	tmpbuf[64], *strptr, *tokptr;
	int  trunk_id;
	rtk_portmask_t	trunk_map;
	rtk_trunk_hashVal2Port_t hash_val;
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		strptr = tmpbuf;
		tokptr = strsep(&strptr, " ");
		if (tokptr == NULL) return len;

		trunk_id = simple_strtol(tokptr, NULL, 0);

		tokptr = strsep(&strptr, " ");
		if (tokptr == NULL) return len;

		trunk_map.bits[0] = simple_strtol(tokptr, NULL, 0);
		rtk_trunk_port_set(trunk_id, trunk_map);

		hash_val.value[0] = 0x6;  /* SMAC & DMAC */
		rtk_trunk_distributionAlgorithm_set(0xff, hash_val);
	}
...
#ifdef CONFIG_RTL_8370_SUPPORT
static void set_giga_phy_config(int phy, int type)
{
        uint32 phyData;
        uint32 phyEnMsk0;
        uint32 phyEnMsk4;
        uint32 phyEnMsk9;
	int32 AutoNegotiation;

        phyEnMsk0 = 0;
        phyEnMsk4 = 0;
        phyEnMsk9 = 0;

        if (type == HALF_DUPLEX_10M || type == PORT_AUTO)
        {
        ...
        if(type == DUPLEX_10M || type == PORT_AUTO)
        {
        ...
        if(type == HALF_DUPLEX_100M || type == PORT_AUTO)
        {
        ...
        if(type == DUPLEX_100M || type == PORT_AUTO)
        {
        ...
        if(type == HALF_DUPLEX_1000M || type == PORT_AUTO)
        {
        ...
        if(type == DUPLEX_1000M || type == PORT_AUTO)
        {
                /*1000 BASE-T FULL duplex capable setting in reg 9.9*/
                phyEnMsk9 = phyEnMsk9 | (1<<9);
                /*100BASE-TX full duplex capable in reg 4.8*/
        ...
        rtl8651_getAsicEthernetPHYReg(phy, 9, &phyData); // PHY_1000_BASET_CONTROL_REG
        phyData = (phyData & (~0x0200)) | phyEnMsk9 ;
        rtl8651_setAsicEthernetPHYReg(phy, 9, phyData); // PHY_1000_BASET_CONTROL_REG


        /*Auto-Negotiation control register setting*/
        rtl8651_getAsicEthernetPHYReg(phy, 4, &phyData); // PHY_AN_ADVERTISEMENT_REG
        phyData = (phyData & (~0x0DE0)) | phyEnMsk4;
        rtl8651_setAsicEthernetPHYReg(phy, 4, phyData); // PHY_AN_ADVERTISEMENT_REG


        /*Control register setting and restart auto*/
        rtl8651_getAsicEthernetPHYReg(phy, 0, &phyData); // PHY_CONTROL_REG
        phyData = (phyData & (~0x3140)) | phyEnMsk0;

        /*If have auto-negotiation capable, then restart auto negotiation*/
        if(AutoNegotiation == 1)
                phyData = phyData | (1 << 9);
        rtl8651_setAsicEthernetPHYReg(phy, 0, phyData); // PHY_CONTROL_REG
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "port", 4))
		{
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT)
#ifdef CONFIG_RTL_8370_SUPPORT
			if ((1<<8) & port_mask) // WAN port
				set_giga_phy_config(P0_EXT_PHY_ID, type);
			else 
#endif
			...
			for(port = 5; port < 9; port++)
			{
				...
				if (((1<<port) & port_mask) && (type != HALF_DUPLEX_1000M))
				{
					set_8367r_speed_mode(port-5, type);
				}
			}
			...
			for(port = 0; port < CPU; port++)
			{
				if((1<<port) & port_mask)
				{
					...
					if(type == PORT_DOWN)
					{
						...
						PCR = REG32(PCRP0+(port<<2));
						PCR &= ~EnablePHYIf;
						REG32(PCRP0+(port<<2)) = PCR;
						continue;
					}
					else
					{
					...
					if(port == 0)
					{
						...
						{
							printk("Port0 -> Force\n");
							port0_port_config = 0x80000000; /* Config exist  */
						}
					}
...
int set_port_led(int flag)
{
		...
		{
			int data32=READ_MEM32(PIN_MUX_SEL_2);
			data32 &= ~MUX2_PORTS_GPIO_VALUE;
                	WRITE_MEM32(PIN_MUX_SEL_2, data32);
		}
#else
...
#else
#if defined CONFIG_EFM_EXTENDER_U1 || defined CONFIG_EFM_EXTENDER_N2 
                WRITE_MEM32(PIN_MUX_SEL_2, 0x3600);
#else
                WRITE_MEM32(PIN_MUX_SEL_2, 0x0);
#endif
#endif
...
#ifdef CONFIG_RTL_8881A
		{
			/* Extender's LAN PORT0 LED is GPIOB2:  1->OFF */
			/* B2 , B3, B4, B5, B6  */

			int data32=READ_MEM32(PIN_MUX_SEL_2);
			data32 |= MUX2_PORTS_GPIO_VALUE;
                	WRITE_MEM32(PIN_MUX_SEL_2, data32);
			/* 2nd bytes is B GPIO: B2~B6 control: */        
		        RTL_W32(PABCD_CNR, (RTL_R32(PABCD_CNR) & (~GPIO_PORTS_LED_MASK)));
		        RTL_W32(PABCD_DIR, (RTL_R32(PABCD_DIR) | GPIO_PORTS_LED_MASK));
		        RTL_W32(PABCD_DAT, (RTL_R32(PABCD_DAT) | GPIO_PORTS_LED_MASK));
			//printk("PABCD : (MUX:%08x) %08x %08x %08x\n", READ_MEM32(PIN_MUX_SEL_2), (unsigned int)RTL_R32(PABCD_CNR),(unsigned int)RTL_R32(PABCD_DIR),(unsigned int)RTL_R32(PABCD_DAT));	
		}
#else
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT) || defined(CONFIG_RTL8367R_VB_API)
...
#ifdef CONFIG_RTL_8370_SUPPORT
		{
			trunk_entry = create_proc_entry("trunk",0,rtl865x_proc_dir);
			if(trunk_entry != NULL)
			{
				trunk_entry->read_proc = trunk_read;
				trunk_entry->write_proc= trunk_write;

				retval = SUCCESS;
			}
			else
			{
				rtlglue_printf("can't create proc entry for trunk");
				retval = FAILED;
				goto out;
			}
		}
